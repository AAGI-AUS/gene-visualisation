...
- `barTicks` generates ticks from each bar's own `p1`/`bpLen`; `collectTicks` (utils.ts) then joins base and query ticks by `chr`+`bp`. A bp on both sides is a connector candidate, one on a single side (a chr the other row lacks, or a bp past the other bar's end) still gets a label on its own side.
...
- Every scale run is labelled at its last row, below the bar, whatever `boundaryTicks` says - a run is never left for the reader to extrapolate from a neighbouring run's scale. A multi-row run is also headed above its first bar. `boundaryTicks` (toolbar `Intra ticks`, default on) adds the rows in between. A pair renders top labels when `isFirst || (boundaryTicks && startsScale && !endsScale)` and bottom labels when `nextEndsScale || (boundaryTicks && sameScale)`, where `startsScale`/`endsScale` are the run edges of its base row and `nextEndsScale` of its query row. The `!endsScale` term is what keeps a one-row run to the bottom placement alone, so no row ever carries both label sets.
