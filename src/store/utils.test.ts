...
describe("buildPalette", () => {
  ...
  it("deduplicates selectedChr if it also appears in the chromosome iterable", () => {
  ...
  it("cycles colors when chromosomes exceed the palette length", () => {
...
describe("computeCommonIds", () => {
  it("returns an empty set when given no pairs", () => {
    expect(computeCommonIds([])).toEqual(new Set());
  });

  it("returns an empty set when pairs share no ids", () => {
    const pairs = [{ rows: [makeRow(1, "1A")] }, { rows: [makeRow(2, "1A")] }];
    expect(computeCommonIds(pairs, 0)).toEqual(new Set());
  });

  it("keeps ids present in every pair when threshold is 0", () => {
    const pairs = [
  ...
  it("drops ids whose query chromosome falls below the threshold in any pair", () => {
  ...
  it("uses the default COMMON_CHR_THRESHOLD when none is supplied", () => {
